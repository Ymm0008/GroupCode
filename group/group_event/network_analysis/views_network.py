...
@mod.route('/representive_text', methods=['POST','GET']) 
...
def related_men(event_type, list0):
    '''该函数用于搜索  人物排名  中得到的影响力比较高的用户的关联用户（转发评论过他和被他转发评论过的），同时找出他们的影响力
...
@mod.route('/related_men_typical_text', methods=['POST','GET']) 
...
@mod.route('/create_es_table', methods=['POST','GET'])
def create_es_table():
	'''在数据库中建立网络分析表，存储网络分析的历史数据,只需创建一次即可
	'''
    index_info = {
        'settings': {
...
@mod.route('/save_search_data', methods=['POST','GET'])
def save_search_data(event_type,list_of_media_rank, list_of_men_rank, list_of_related_men, 
	                list_of_media_representitive_blog, list_of_men_representitive_blog,
	                list_of_related_men_representitive_blog):
    '''将计算之后的数据存入数据库的网络分析表中
    '''
    es.index(index='network_analysis', doc_type = event_type,         
             body={'media_rank': list_of_media_rank , 'man_rank': list_of_men_rank, 
              'related_men': list_of_related_men,
              'representitive_blog_of_men': list_of_men_representitive_blog, 
              'representitive_blog_of_meida':list_of_media_representitive_blog, 
              'representitive_blog_in_the_relationship': list_of_related_men_representitive_blog, 
...
@mod.route('/timestamp_change', methods=['POST','GET']) 
