...
def network_analysis(index_name, event_name, es):
    ...
    def calculate_influence(index_name, event_name, uid_input):
    ...
    def influence_rank(index_name, event_name, get_data_num):
    ...
    def influence_rank_classified(total_influence_rank):
    ...
    def representive_text(index_name, event_name, list_of_rank): 
        ...
        for i in range(len(list_of_blog)): #给每个uid加上名字
            list_of_blog[i]['nick_name'] = get_uid_name(list_of_blog[i]['uid'])['nick_name']
    ...
    def related_men(index_name, event_name, list0):
        ...
        for i in range(len(lista)):                        #遍历每一个活跃度比较高的用户
            for j in range(len(lista[i]['related_men'])):   #遍历一个用户的每个关联用户，得到其影响力
                list3.append(calculate_influence(index_name,event_name,lista[i]['related_men'][j]))
            if list3 != []:              
    ...
    def related_men_typical_text(index_name, event_name, listC):
        ...
        for i in range(len(listC)):  #遍历每一个表格中的关联用户
            ...
            list0 = representive_text(index_name, event_name, list1[i]['related_men']) 
            if list0 != []: 
    ...
    def save_search_data(event_name,list_of_media, list_of_men, list_of_media_blog, list_of_men_blog,\
    ...
    if __name__ == '__main__':
        listA = influence_rank(index_name, event_name, 100)
        list_of_media, list_of_men = influence_rank_classified(listA) 
        list_of_media_blog = representive_text(index_name, event_name, list_of_media)
        list_of_men_blog = representive_text(index_name, event_name, list_of_men)
        list_of_related_men = related_men(index_name, event_name, listA)      #引入listA算法
        list_of_related_men_blog = related_men_typical_text(index_name, event_name, list_of_related_men) #调用listA算法     
        list_of_data_for_graph = data_for_graph(list_of_related_men)
        save_search_data(event_name, list_of_media, list_of_men, list_of_media_blog, list_of_men_blog,list_of_related_men, list_of_related_men_blog)

if __name__ == '__main__':
    es = Elasticsearch('219.224.134.226:9207', timeout=1200)
    network_analysis('flow_text_gangdu', 'text', es)
