...
AB_PATH = os.path.join(os.path.dirname(__file__), 'data')
...
emotion_pattern = re.compile(r'\[(\S+?)\]')    # [汗] 表情
...
def triple_classifier(weibo_text):
    '''
    输出结果：
    0 中性
    1 积极
...
if __name__ == '__main__':
    ...
    start = time.time()
    result = []
    for uid,text in uid_weibo.iteritems():
