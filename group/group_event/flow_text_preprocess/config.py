...
abs_path = os.path.join(os.path.dirname(__file__), 'dict')
...
ABSOLUTE_DICT_PATH = os.path.join(abs_path, '/dict')
...
def load_dict(emotion_list):
    ...
    for i in range(0,len(emotion_list)):
        ...
        reader = csv.reader(file(abs_path+'/%s.csv'% emotion_list[i], 'rb'))
        for line in reader:
            word = line[0].strip('\r\n')
            w_set.append(word)
